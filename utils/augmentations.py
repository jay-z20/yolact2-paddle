...
class ToTensor(object):
    def __call__(self, cvimage, masks=None, boxes=None, labels=None):
        return paddle.to_tensor(cvimage.astype(np.float32)).transpose((2, 0, 1)), masks, boxes, labels
...
class FastBaseTransform(paddle.nn.Layer):
    ...
    def __init__(self):
        ...
        self.mean = paddle.to_tensor(np.array(MEANS)[None, :, None, None],dtype='float32')  #.cuda()[None, :, None, None]
        self.std  = paddle.to_tensor(np.array(STD)[None, :, None, None],dtype='float32')#[None, :, None, None]  #.cuda()[None, :, None, None]
    ...
    def forward(self, img):
        ...
        if cfg.preserve_aspect_ratio: # FALSE
            _, h, w, _ = img.size()
        ...
        img = img.transpose((0, 3, 1, 2))#.contiguous()
        img = F.interpolate(img, img_size,scale_factor=None, mode='bilinear', align_corners=False)
        ...
        img = paddle.index_select(img,paddle.to_tensor([2,1,0]),1)
